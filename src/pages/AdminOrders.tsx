...
const AdminOrders = () => {
  ...
  const groupedOrders = {
    today: filteredOrders.filter(order => order.date === today),
    yesterday: filteredOrders.filter(order => order.date === yesterday),
    older: filteredOrders.filter(order => order.date !== today && order.date !== yesterday)
  };
  
  const renderOrdersTable = (ordersList: Order[], title: string) => {
  ...
  return (
    <AdminLayout title="Orders">
      ...
      {filteredOrders.length === 0 ? (
        ...
        <>
          {renderOrdersTable(groupedOrders.today, "Today's Orders")}
          {renderOrdersTable(groupedOrders.yesterday, "Yesterday's Orders")}
          {renderOrdersTable(groupedOrders.older, "Older Orders")}
        </>
      )}
    </AdminLayout>
  );
