        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <div className="flex items-center mb-6">
              <span className="text-3xl font-bold text-smartplug-blue">{getCurrentPrice()} {t('currency')}</span>
              {getCurrentOldPrice() && (
                <span className="line-through text-gray-500 ml-2">{getCurrentOldPrice()} {t('currency')}</span>
              )}
            ...
            {product.colorVariants && product.colorVariants.length > 0 && (
              <div className="mb-6">
                <h3 className="text-sm font-medium mb-3">{t('selectColor')}</h3>
                ...
                {selectedColorVariant && (
                  <p className="text-sm mt-2">
                    ...
                    {selectedColorVariant.priceAdjustment > 0 && ` (+${selectedColorVariant.priceAdjustment} ${t('currency')})`}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="flex items-center mb-6">
              <div className="flex items-center">
                <span className="text-sm font-medium">
                  {getCurrentStock() > 0 
                    ? `${t('inStock')} (${getCurrentStock()} ${t('available')})`
                    : t('outOfStock')}
                </span>
              </div>
              <div className="ml-6 text-sm text-gray-600">
                {t('sku')}: <span className="font-medium">{product.sku}</span>
              </div>
            </div>
          ...
          <Tabs defaultValue="description">
            ...
            <div className="p-4 border rounded-b-lg bg-white">
              <TabsContent value="description">
                <h3 className="text-lg font-medium mb-2">{t('productDescription')}</h3>
